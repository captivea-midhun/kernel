...
class ReportProjectedBomStructure(models.AbstractModel):
    _name = 'report.kernel_projected_bom_cost.projected_bom_structure'
    _description = 'Projected BOM Structure Report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for bom_id in docids:
            ...
            if not candidates:
                if data and data.get('childs'):
                    doc = self._get_pdf_line(
                        bom_id, qty=quantity, child_bom_ids=json.loads(data.get('childs')))
                else:
    ...
    def get_operations(self, bom_id=False, qty=0, level=0):
        ...
        lines = self._get_operation_line(
            bom.routing_id, float_round(
                qty / bom.product_qty, precision_rounding=1, rounding_method='UP'), level)
    ...
    def _get_report_data(self, bom_id, searchQty=0, searchVariant=False):
        ...
        return {'lines': lines,
                ...
                'is_variant_applied':
                    self.env.user.user_has_groups('product.group_product_variant')
    ...
    def _get_bom(self, bom_id=False, product_id=False, line_qty=False, line_id=False, level=False):
        ...
        if bom.product_qty > 0:
            operations = self._get_operation_line(
                bom.routing_id, float_round(bom_quantity / bom.product_qty,
                                            precision_rounding=1, rounding_method='UP'), 0)
        company = bom.company_id or self.env.company
        proposed_cost = self._get_proposed_cost(product, bom_quantity)

        bom_price = 0.0
        bom_total = 0.0
        if bom.type == 'subcontract':
            bom_price = product.uom_id._compute_price(proposed_cost, bom.product_uom_id) * bom_quantity
            bom_total = sum([op['total'] for op in operations]) + bom_price
        else:
            bom_price = product.uom_id._compute_price(proposed_cost, bom.product_uom_id) * bom_quantity
            bom_total = sum([op['total'] for op in operations])
    ...
    def _get_bom_lines(self, bom, bom_quantity, product, line_id, level):
        ...
        for line in bom.bom_line_ids:
            ...
            # proposed_cost = self._get_proposed_cost(line.product_id, bom_quantity)
            proposed_cost = self._get_proposed_cost(line.product_id, line_quantity)
            price = line.product_id.uom_id._compute_price(
                proposed_cost, line.product_uom_id) * line_quantity
            ...
            if line.child_bom_id:
                ...
                if line.child_bom_id.type == 'subcontract':
                    sub_total = self._get_price(line.child_bom_id, factor, line.product_id) + price
                else:
    ...
    def _get_operation_line(self, routing, qty, level):
        ...
        for operation in routing.operation_ids:
            operation_cycle = float_round(qty / operation.workcenter_id.capacity,
                                          precision_rounding=1, rounding_method='UP')
            duration_expected = \
        ...
        for line in bom.bom_line_ids:
            ...
            if line.child_bom_id:
                ...
                if line.child_bom_id.type == 'subcontract':
                    ...
                    company = bom.company_id or self.env.company
                    # proposed_cost = self._get_proposed_cost(line.product_id, line.product_qty)
                    proposed_cost = self._get_proposed_cost(line.product_id, prod_qty)
                    not_rounded_price = line.product_id.uom_id._compute_price(
                        proposed_cost, line.product_uom_id) * prod_qty
                    sub_price += company.currency_id.round(not_rounded_price)
        ...
        def get_sub_lines(bom, product_id, line_qty, line_id, level):
            ...
            for bom_line in bom_lines:
                ...
                if bom_line['child_bom'] and (unfolded or bom_line['child_bom'] in child_bom_ids):
                    ...
                    lines += (get_sub_lines(
                        line.child_bom_id, line.product_id, bom_line['prod_qty'], line, level + 1))
            if data['operations']:
                lines.append({
                    'name': _('Operations'),
                    'type': 'operation',
    ...
    def _get_proposed_cost(self, product, bom_quantity):
        proposed_cost, seller_ids = 0.0, product.seller_ids
        if seller_ids:
            # vendor_ids = seller_ids.filtered(lambda r: r.min_qty == bom_quantity)
            vendor_ids = seller_ids.filtered(lambda r: r.min_qty <= bom_quantity)
            vendor_ids = sorted(vendor_ids, key=lambda r: r.min_qty, reverse=True)
            # bom_quantity_new = bom_quantity
            # if not vendor_ids:
            #     flag = False
            #     while not flag:
            #         bom_quantity_new = bom_quantity_new - 1
            #         vendor_ids = seller_ids.filtered(lambda r: r.min_qty == bom_quantity_new)
            #         if vendor_ids:
            #             flag = True
            #         if bom_quantity_new == 0:
            #             break
            # proposed_cost = 0.0
            if vendor_ids:
                # self._cr.execute("select min(price) from product_supplierinfo where id in %s",
                #                  (tuple(vendor_ids.ids),))
                self._cr.execute("select min(price) from product_supplierinfo where id in %s",
                                 (tuple(vendor_ids[0].ids),))
                min_price = list(filter(None, map(lambda x: x[0], self._cr.fetchall())))
                if min_price:
                    proposed_cost = min_price[0]

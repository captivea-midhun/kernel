...
class Report1099(models.Model):
    ...
    type_income = fields.Selection(income_list, string="Income Type", default='non_emp_cmpr')
    amount = fields.Float()
    ssn_ein = fields.Selection([('ssn', 'SSN'), ('ein', 'EIN')], string='SSN/EIN No.')
    legal_name = fields.Char()
    paid_date = fields.Date()
    paid_ref = fields.Char('Paid Reference')
    invoice_state = fields.Selection([('open', 'Open'), ('paid', 'Paid')], string='Invoice Status')
    check_number = fields.Integer()
    invoice_line_id = fields.Many2one('account.move.line', 'Invoice Line Ref.', copy=False)

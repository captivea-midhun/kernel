...
class ResPartner(models.Model):
    ...
    is_1099 = fields.Boolean(track_visibility='onchange')
    ...
    type_income = fields.Selection(
        income_list, string="Income Type", default='non_emp_cmpr', track_visibility='onchange',
        groups="report_1099.group_report_1099_vendor, report_1099.group_report_1099")
    type_co = fields.Selection(
        [('individual_sole', 'Individual/Sole Proprietor'),
         ...
         ], string='Federal Tax Classification', track_visibility='onchange',
        groups="report_1099.group_report_1099_vendor, report_1099.group_report_1099")
    legal_name = fields.Char(
        'TIN Name', track_visibility='onchange',
        groups="report_1099.group_report_1099_vendor, report_1099.group_report_1099")
    ...
    def read(self, fields=None, load='_classic_read'):
        result = super(ResPartner, self).read(fields, load=load)
        for record in result:
            if record.get('ssn_number'):
                record['ssn_number'] = 'XXX-' + 'XX-' + str(record['ssn_number'])[-4:]
    ...
    @api.model
    def check_and_fill_legal_name(self):
        """
        This method will be called during module Upgrade / Install time and
        will fill up the value of legal name for vendors if its blank.
        """
        self._cr.execute("""update res_partner set legal_name = name where
                             legal_name IS NULL and supplier_rank > 0
                             and is_1099 IS TRUE""")

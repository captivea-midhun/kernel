...
from odoo.addons.ocplm_odoo_api.common import invalid_response, valid_response
from odoo.http import request

_logger = logging.getLogger(__name__)

expires_in = "ocplm_odoo_api.access_token_expires_in"
...
class AccessToken(http.Controller):
    ...
    @http.route(
        "/api/login", methods=["POST"], type="http", auth="none", csrf=False
    )
    def token(self, **post):
        ...
        try:
            ...
            _logger.error(info)
            return invalid_response("wrong database name", error, 404)
        ...
        data={
            ...
            "name":request.env.user.partner_id.name or '',
            "code":200,
        }
        # Successful response:
